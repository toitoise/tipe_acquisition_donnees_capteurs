...
//	Wire.write(bin2bcd(epoch_tm.tm_wday));	//0x03 Week Day
	Wire.write(bin2bcd(epoch_tm.tm_wday+1));	//0x03 Week Day => ST CORRECTION do +1
...
time_t DS3231::getEpoch()
{
	...
	epoch_tm.tm_wday = bcd2bin(Wire.read())-1;	//0x03 Week Day 
	...
	epoch_tm.tm_yday = 0;		// not used for epoch
	epoch_tm.tm_isdst = 0;		// no DST shift
